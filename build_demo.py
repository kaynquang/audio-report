...
<main>
  ...
  <div class="card">
    ...
    <div class="note"><b>Lưu ý về giá Gemini:</b> con số $1/$20 là <b>giá NIÊM YẾT của Google</b> (<a href="https://ai.google.dev/gemini-api/docs/pricing" target="_blank">nguồn</a>). Giá <b>thực tế qua proxy LiteLLM của Athena</b> tuỳ cấu hình admin — <b>chưa verify được</b> (API key bị giới hạn, không đọc được <code>/model/info</code>); hơn nữa bảng giá mặc định của LiteLLM <b>chưa có</b> model này, nên proxy chỉ tính đúng nếu admin cấu hình cost override. → Hỏi admin proxy để biết giá thật.</div>
